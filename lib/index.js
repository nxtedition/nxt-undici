...
export { Client, Pool, Agent, setGlobalDispatcher } from '@nxtedition/undici'

// Guarded re-export instead of the fork's raw getGlobalDispatcher — see
// getDefaultDispatcher below for the node>=26.3 undici-v8 symbol collision.
...
function getDefaultDispatcher() {
  ...
  // Node >= 26.3 bundles undici v8, which registers the global fetch
  // dispatcher under the same Symbol.for('undici.globalDispatcher.2') slot
  // the @nxtedition/undici fork claims. Any fetch() call that runs before the
  // fork's module init (or that re-initializes node's fetch machinery) parks
  // node's built-in Agent in the shared slot; that Agent only accepts the new
  // controller-style handler API, so every pipeline dispatch would then be
  // rejected with "invalid onRequestStart method". Only trust the slot when
  // it actually holds a fork dispatcher; otherwise fall back to our own
  // process-wide Agent.
  if (dispatcher instanceof undici.Dispatcher) {
    return dispatcher
  }
  return (fallbackDispatcher ??= new undici.Agent())
}
