...
export const interceptors = {
  ...
  trace: (await import('./interceptor/trace.js')).default,
...
function wrapDispatch(dispatcher) {
  ...
  if (wrappedDispatcher == null) {
    wrappedDispatcher = compose(
      ...
      interceptors.log(),
      interceptors.lookup(),
      // Later entries in this list wrap earlier ones, so trace sits OUTSIDE
      // everything that does real work (lookup, log, redirect, cache, proxy,
      // retry, dns) — durationMs measured here spans the whole composed
      // pipeline including retries, dns and cache lookups — but INSIDE
      // requestId, so opts.id is already stamped and the emitted docs
      // correlate with logs and undici:retry docs by request id.
      interceptors.trace(),
        ...
        return dispatch(
          {
            ...
            // Undefined-check (not ??): an explicit `trace: null` disables tracing
            // for the request; only an absent option falls back to the per-thread
            // writer (see lib/trace.js).
            trace: opts.trace === undefined ? globalThis.__nxt_lib_trace : opts.trace, // Avoid looking up __nxt_lib_trace on every trace call.
            dns: opts.dns ?? true,
