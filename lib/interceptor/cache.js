...
export default () => (dispatch) => (opts, handler) => {
  ...
  return serveFromCache(entry ?? { statusCode: 504 }, opts, handler, write, url, lookupMs)
}

function serveFromCache(entry, opts, handler, write, url, lookupMs) {
  ...
  // Everything served here — entry, conditional 304 and only-if-cached 504 —
  // is a lookup hit. Emitted before onConnect and outside the try/catch below
  // so trace code sits strictly outside the handler-contract enforcement
  // (traceSafe cannot throw, but keep it out of that window anyway). The
  // only-if-cached 504 has no cachedAt, so its ageSec stays null.
  if (write !== null) {
    traceLookup(write, opts, url, 'hit', null, statusCode, age, body?.byteLength ?? 0, lookupMs)
  }
