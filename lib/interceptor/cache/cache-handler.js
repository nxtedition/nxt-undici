...
export class CacheHandler extends DecoratorHandler {
  ...
  onHeaders(statusCode, headers, resume) {
    ...
    if (headers['content-range']) {
      ...
      if (
        contentRange == null ||
        // An open-ended `bytes N-/M` (no last-byte-pos) is not a valid
        // byte-range-resp: parseContentRange yields end === null, which the
        // previous `end != null &&` guard let slip through, so a malformed
        // partial (e.g. `bytes 15-/10`, start past size) was stored and could
        // later be replayed for a Range request. Treat a missing end as invalid.
    ...
    // Authorization only when the response explicitly allows it with a
    // syntactically valid public, s-maxage, or must-revalidate directive.
    // Must stay in lockstep with the serve-side gate in the interceptor
    // (index.js). A duplicated (array) authorization header is refused.
    const authorization = this.#key.headers.authorization
    if (authorization != null) {
      if (
        typeof authorization !== 'string' ||
        !allowsAuthenticatedCaching(cacheControlDirectives)
      ) {
    ...
    if (
      (cacheControlDirectives['no-store'] && cacheControlDirectives['must-understand'] !== true) ||
    ...
    if (cacheControlDirectives['must-understand']) {
      // By this point the status gate above has limited storage to response
      // codes whose caching requirements this implementation understands.
      // RFC 9111 §5.2.2.3 therefore says to ignore a paired no-store, which is
      // the fallback for caches that do not implement must-understand.
    }
    ...
    if (end == null || end - start <= this.#maxEntrySize) {
      ...
      this.#value = {
        ...
        etag,
        vary,
        // RFC 9111 §3.5 constrains every later reuse based on the request
        // that produced this response, not only on the request doing the
        // lookup. Persist that provenance because a 304 can replace
        // Cache-Control and remove the directive that originally permitted
        // shared caching.
