...
export function determineLifetime(
  ...
  if (sMaxAge != null) {
    // `false` is parseCacheControl's restrictive sentinel for a present but
    // malformed s-maxage. It makes the response stale without masquerading as
    // the valid directive that permits shared caching under Authorization.
    return { lifetime: typeof sMaxAge === 'number' ? sMaxAge : 0, explicit: true }
  }
  const maxAge = cacheControlDirectives['max-age']
  if (maxAge != null) {
    return { lifetime: maxAge, explicit: true }
  }
...
export function forbidsServingStale(entry) {
  ...
  return directives?.['must-revalidate'] != null || directives?.['proxy-revalidate'] != null
}

/**
 * RFC 9111 §3.5: only syntactically valid response directives can permit a
 * shared cache to store/reuse a response to an authenticated request.
 * parseCacheControl represents malformed restrictive forms as `false`, so
 * exact type/value checks are security-significant here.
 */
export function allowsAuthenticatedCaching(directives) {
  return (
    directives?.public === true ||
    typeof directives?.['s-maxage'] === 'number' ||
