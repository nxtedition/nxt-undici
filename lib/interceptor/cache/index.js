...
export default ({ origins } = {}) => {
  ...
  return (dispatch) => (opts, handler) => {
    ...
    // RFC 9111 §3.5 ties the permission to the request that produced the
    // STORED response. If a later 304 replaced Cache-Control and withdrew the
    // grant, that response must not satisfy any request — including an
    // anonymous one. This provenance gate is intentionally separate from the
    // current-request gate below so the latter can continue mirroring the
    // CacheHandler storage policy.
    if (entry?.authorizationRequest === true) {
      const directives = entry.cacheControlDirectives
      if (!allowsAuthenticatedCaching(directives)) {
        entry = undefined
        missReason = 'auth'
      }
    }

    // Serve-side policy for the CURRENT request: a shared cache does not use
    // an entry for a request with Authorization unless the stored response
    // allows it through a syntactically valid public, s-maxage, or
    // must-revalidate directive. This mirrors the store-side CacheHandler
    // gate; both sites must stay in lockstep.
    if (entry && headers.authorization != null) {
      const directives = entry.cacheControlDirectives
      if (typeof headers.authorization !== 'string' || !allowsAuthenticatedCaching(directives)) {
        entry = undefined
        missReason = 'auth'
      }
    }
