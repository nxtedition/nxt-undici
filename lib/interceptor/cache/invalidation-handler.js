...
export class InvalidationHandler extends DecoratorHandler {
  ...
  #invalidate(headers) {
    ...
    for (const name of ['location', 'content-location']) {
      ...
      // Build the base by concatenating origin + path via buildURL rather than
      // `new URL(path, origin)`: the latter treats a protocol-relative request
      // path (e.g. `//api/items/1`, reachable via a request URL like
      // `http://host//api/items/1`) as an authority and discards the real
      // origin. A same-origin Location would then be judged cross-origin and
      // its invalidation silently skipped (stale-after-write), while a sibling
      // path on a spoofed origin could slip through. See redirect.js, which
      // guards the identical hazard. buildURL keeps the request origin
      // authoritative.
