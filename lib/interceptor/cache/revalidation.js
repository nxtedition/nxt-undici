...
export class RevalidationHandler {
  ...
  #freshen() {
    ...
    try {
      ...
      const authorizationRequest = entry.authorizationRequest
      // An old/custom store might not round-trip the new provenance field.
      // A transition from a present §3.5 grant to no grant is nevertheless
      // enough evidence to fail closed. This can conservatively retire an
      // anonymous legacy entry, but caching is optional and leaking an
      // authenticated representation is not.
      const authorizationGrantWithdrawn =
        !allowsAuthenticatedCaching(cacheControlDirectives) &&
        (authorizationRequest === true ||
          (authorizationRequest == null &&
            allowsAuthenticatedCaching(entry.cacheControlDirectives)))
      if (
        (cacheControlDirectives['no-store'] &&
          cacheControlDirectives['must-understand'] !== true) ||
        cacheControlDirectives.private === true ||
        authorizationGrantWithdrawn
      ) {
      ...
      const freshened = {
        ...
        etag,
        vary,
        // A 304 updates metadata for the same stored response; it does not
        // erase the provenance of the request that supplied its body.
