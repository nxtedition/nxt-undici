...
export default () => (dispatch) => {
  ...
  return async (opts, handler) => {
    try {
      ...
      if (record == null) {
        // Sort a copy — concurrent requests share the cached array and
        // balance:'hash' relies on a stable index.
        const sorted = records
          .slice()
          .sort((a, b) => a.errored - b.errored || a.pending - b.pending || a.counter - b.counter)

        for (let i = 0; i < sorted.length; i++) {
          if (sorted[i].expires >= now && sorted[i].timeout < now) {
            record = sorted[i]
            break
          }
        }
      ...
      if (record == null) {
        // Every record is in cooldown but at least one has a valid expires.
        // Force a re-resolve: fresh records have timeout:0, so either we
        // discover new IPs or reset state on the same ones — beats hard-
        // failing with ENOTFOUND until expires passes.
        const [err, val] = await resolve(hostname, { ttl })
        if (err) {
          throw err
        }
