...
const globalArray = (globalThis.__undici_requests ??= [])
const globalIndexMap = new WeakMap()
...
class Handler extends DecoratorHandler {
  ...
  constructor(logOpts, opts, { handler }) {
    ...
    globalIndexMap.set(this, globalArray.push(this) - 1)
  }
  ...
  onDone() {
    const idx = globalIndexMap.get(this)
    if (idx !== undefined) {
      globalIndexMap.delete(this)
      const tmp = globalArray.pop()
      if (tmp !== this) {
        globalArray[idx] = tmp
        globalIndexMap.set(tmp, idx)
      }
    }
  }
}
