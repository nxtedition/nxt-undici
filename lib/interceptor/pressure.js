...
function validateNumber(name, value, predicate, range) {
  if (typeof value !== 'number') {
    throw new TypeError(`opts.${name} must be a number`)
  }
  if (!Number.isFinite(value) || !predicate(value)) {
    throw new RangeError(`opts.${name} must be ${range}`)
  }
}

function validateHysteresis(name, lo, hi) {
  const range = 'a finite number strictly between 0 and 1'
  validateNumber(`${name}Lo`, lo, (value) => value > 0 && value < 1, range)
  validateNumber(`${name}Hi`, hi, (value) => value > 0 && value < 1, range)
  if (lo >= hi) {
    throw new RangeError(`opts.${name}Lo must be less than opts.${name}Hi`)
  }
}
...
class PressureMonitor {
  ...
  constructor({
    ...
    validateNumber(
      'sampleInterval',
      sampleInterval,
      (value) => value >= 0 && value <= MAX_TIMER_DELAY,
      `a finite number from 0 through ${MAX_TIMER_DELAY}`,
    )
    validateNumber('tau', tau, (value) => value > 0, 'a finite number greater than 0')
    // The transitions use strict > / < comparisons. Endpoint thresholds would
    // therefore make engagement (hi=1) or release (lo=0) unreachable.
    validateHysteresis('some', someLo, someHi)
    validateHysteresis('full', fullLo, fullHi)
    validateHysteresis('err', errLo, errHi)
