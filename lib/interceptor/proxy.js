...
function reduceHeaders({ headers, proxyName, httpVersion, socket }, fn, acc) {
  ...
  if (Array.isArray(connection)) {
    ...
    // Single value. The isHopByHop guard skips the common single-token forms
    // (`keep-alive`, `close`, …) where there is nothing custom to strip.
    remove = connection.split(',').map((s) => s.trim().toLowerCase())
  }
