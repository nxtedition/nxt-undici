...
class Handler extends DecoratorHandler {
  ...
  #maybeRetry(err) {
    if (
      ...
      // or other passthrough statuses) or #end is not positive: zero means
      // nothing left to request (`bytes=0--1` is invalid), negative means the
      // server sent a bogus content-length (e.g. `-5`). A non-positive #end
      // can enter both from the initial response and from a full-200 restart,
      // so guard here at the single resume decision point, mirroring the
      // precondition asserted below. Without this, the resume asserts would
      // throw and be delivered to the user IN PLACE of the original error.
