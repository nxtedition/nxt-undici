    ...
    if (signal?.aborted) {
      ...
      const onAbort = () => {
        ...
        if (this.res) {
          ...
          // A dispatcher can queue a request without calling onConnect (for
          // example, an Agent at its connection/pipelining limit). Settle the
          // public promise immediately; when the queued transport eventually
          // calls onConnect, the aborted branch below still cancels it before
