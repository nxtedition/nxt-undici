    ...
    try {
      ...
      for (let retryCount = 0; true; retryCount++) {
        ...
        try {
          ...
          // ROLLBACK is required: a failed INSERT/COMMIT leaves the connection
          // with an open transaction; without it the next BEGIN would throw.
          // Skip ROLLBACK if BEGIN itself failed — there is no open transaction.
          if (began) {
          ...
          if (err?.errcode === 13 /* SQLITE_FULL */ && retryCount < 3) {
            ...
            // If BEGIN failed (began=false, n=0) clear the whole batch to avoid
            // an infinite retry loop. If an INSERT/COMMIT failed, only clear the
            // entries that were part of the attempted transaction.
            this.#insertBatch.splice(0, began ? n : this.#insertBatch.length)
            throw err
          }
        }
      }
