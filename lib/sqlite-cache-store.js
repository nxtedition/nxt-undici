...
/**
 * @typedef {import('./index.js').CacheStore} CacheStore
 * @typedef {import('./index.js').CacheKey} CacheKey
 * @typedef {{
 *  id: Readonly<number>,
 *  body?: Uint8Array
 *  start: number
 ...
 *  maxEntrySize?: number,
 *  maxEntryTTL?: number,
 *  db?: ConstructorParameters<typeof DatabaseSync>[1],
 *  store?: CacheStore
...
export class SqliteCacheStore {
  ...
  /**
   * @param {SqliteCacheStoreOptions | undefined} opts
   */
  constructor(opts) {
  ...
  /**
   * @param {CacheKey} key
   * @returns {(import('./index.js').CacheResult & { body?: Buffer }) | undefined}
   */
  get(key) {
    assertCacheKey(key)
  ...
  /**
   * @param {CacheKey} key
   * @param {import('./index.js').CacheValue & { body: null | Buffer | Array<Buffer>, start: number, end: number }} value
   */
  set(key, value) {
    assertCacheKey(key)
  ...
  #findValue(key, canBeExpired = false) {
    ...
    const values = /** @type {SqliteStoreValue[]} */ (
      this.#getValuesQuery.all(makeValueUrl(key), method, range?.start ?? 0)
    )
