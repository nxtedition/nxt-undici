...
export class SqliteCacheStore {
  ...
  #findValue(key) {
    ...
    for (const value of values) {
      if (matchesValue(value, range, headers)) {
        // SQL already excludes expired persisted rows. This check applies to
        // pending tombstones, which must shadow older matching DB rows but
        // must never themselves be returned.
...
function makeResult(value) {
  return {
    ...
    cacheControlDirectives: value.cacheControlDirectives
      ...
      : undefined,
    authorizationRequest:
      value.authorizationRequest == null ? undefined : value.authorizationRequest === 1,
    cachedAt: value.cachedAt,
