...
function versionedLocation(location) {
  ...
  // The current layout treats `location` as a directory holding one file per
  // schema version. mkdirSync(recursive) is idempotent for an existing
  // directory, so an EEXIST here means an older package version left a plain
  // database file *at* `location`. Opening `location/vN` under a file yields
  // SQLITE_CANTOPEN, so remove the stale file (and its sqlite sidecars) and
  // reclaim the path as a directory — version bumps already invalidate old
  // caches.
  try {
    fs.mkdirSync(location, { recursive: true })
  } catch (err) {
    if (err?.code !== 'EEXIST' || !fs.statSync(location).isFile()) {
      throw err
    }
 ...
 *  authorizationRequest?: number | null
 *  cachedAt: number
 *  staleAt: number
 *  deleteAt: number
 * }} SqliteStoreValue
  ...
  set(key, value) {
    ...
    const entry = {
      ...
      cacheControlDirectives: value.cacheControlDirectives
        ...
        : null,
      // NULL deliberately preserves "unknown" for callers/custom-store
      // migrations that predate the provenance field. CacheHandler writes an
      // explicit 0/1 for every new response.
      authorizationRequest:
        value.authorizationRequest == null ? null : value.authorizationRequest ? 1 : 0,
  ...
  #findValue(key) {
    ...
    for (const entry of this.#insertBatch) {
      if (entry.url === url && entry.method === method && entry.start <= requestedStart) {
        ...
        // representation they tombstone (including a 304 that withdraws
        // cacheability or an Authorization grant).
...
function makeResult(value) {
  return {
    ...
    cacheControlDirectives: value.cacheControlDirectives
      ...
      : undefined,
    authorizationRequest:
      value.authorizationRequest == null ? undefined : value.authorizationRequest !== 0,
    cachedAt: value.cachedAt,
...
function assertCacheValue(value) {
  ...
  if (value.authorizationRequest !== undefined && typeof value.authorizationRequest !== 'boolean') {
    throw new TypeError(
      `expected value.authorizationRequest to be boolean, got ${printType(value.authorizationRequest)} [${value.authorizationRequest}]`,
    )
  }
}
