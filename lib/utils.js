...
export function isDisturbed(body) {
  ...
  return stream.Readable.isDisturbed(body)
}
...
export function parseHeaders(headers, obj) {
  ...
  if (obj != null && 'content-length' in obj && 'content-disposition' in obj) {
    const disposition = obj['content-disposition']
    if (Array.isArray(disposition)) {
      obj['content-disposition'] = disposition.map((val) =>
        Buffer.from(String(val)).toString('latin1'),
      )
    } else if (disposition != null) {
      obj['content-disposition'] = Buffer.from(String(disposition)).toString('latin1')
    }
  }
