...
const headerNameLowerCasedRecord = new Map()

export function parseHeaders(headers, obj = {}) {
  for (let i = 0; i < headers.length; i += 2) {
    const rawKey = headers[i].toString()

    let key = headerNameLowerCasedRecord.get(rawKey)
    if (key === undefined) {
      key = rawKey.toLowerCase()
      headerNameLowerCasedRecord.set(rawKey, key)
    }
